...
function ManagerViewCustomer() {
    ...
    useEffect(() => {
      const checkAccount = async () => {
        try {
          const response = await managerService.view_customer_account();
                ...
                <tbody>
                    {accountdata.results?.map((item) => (
                        <tr key={item.accno}>
                            ...
                            <td>{item.accno}</td>
                            <td>{item.balance}</td>
                            <td><button style={{marginTop: '10px'}} className="btn btn-info btn-sm mx-2" onClick={() => navigate("/viewTransactions",{ state: { account_no: item.accno } })}>Transaction History</button></td>
                        </tr>
                    ))}
                </tbody>
        </table>
...
export default ManagerViewCustomer;